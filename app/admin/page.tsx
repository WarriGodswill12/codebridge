...
export default async function AdminOverviewPage() {
  const [projects, services, tiers, posts] = await Promise.all([
  ...
  return (
    <div>
      ...
      <h1 className="mt-2 font-display text-3xl italic">What's live on the site.</h1>
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div className="flex min-h-32 flex-col justify-between rounded-2xl bg-secondary p-6">
          <span className="font-display text-3xl">
            {projects.filter((p) => p.coverImageId).length}
          </span>
          <span className="text-sm text-muted-foreground">with a cover image</span>
        </div>
      </div>
